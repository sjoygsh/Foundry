...
- **Activation only by the baseline a peer was sent.** The server's host calls `sendBaseline` on
  a synchronizing peer: reliable, on the session's full-state channel, never replaced. The
  client's host takes it as a `baseline` delivery and acknowledges it by name (`BaselineRef`:
  sequence and tick). The server activates only on an exact match and answers `active`, and
  each side then reports an `activated` event. Wrong-order and false claims end only that peer.
  A session with no full-state channel cannot activate anyone (`NoStateChannel`); its channels
  still start. `PeerState` gained `active`.
- **Commands only in tick batches.** `sendCommand` (active, a reliable channel running that way)
  returns a number counted from 1 per connection. The server appends arrivals to a bounded
  per-peer inbox (112 KiB at the reference limits); a full inbox ends that peer as `overloaded`.
  `admitBatch(session, tick)` takes each active peer's oldest commands, one per peer per round in
  participant order, up to the new `Limits.commands_per_peer_per_tick` (16), and copies them
  into per-session batch storage sized by `queued_event_payload_bytes`. It then orders them by
  participant and command number. Ticks strictly increase. `batchCommand` and
  `copyBatchPayload` read the batch. A server has no deliveries, so there is no way around
  admission. The rest wait for a later tick; a departed peer keeps what was admitted and loses
  what was not.
- **State replaces state.** `publishState` copies into a slot beside the send queue. It enters
  the queue, and is numbered, only once the last state queued has gone to TLS, so a slow peer
  holds at most two. Published after the baseline and before activation, it waits. A client
  refuses state stamped earlier than the last, and delivers reliable messages in order before
  the newest untaken state (`nextDelivery`/`takeDelivery`).
- **Storage:** each connection buffer is exactly its send plus receive storage. `init` refuses,
  as `QueueTooSmall`, receive storage under three frames plus a record, send storage under two
  frames, or batch storage under one frame. Three events are now reserved per authorized
  connection.
- **Proofs:** `zig build net-session-test`, now 21, gained seven:
  - activation and everything refused before it;
  - initial-sync failures — no acknowledgement, a rejected baseline, six false acknowledgements
    and nine lies from a hand-driven server;
  - three bursting clients whose batches stay ordered, contiguous and in budget, with replayed
    batches rebuilding the reference model exactly;
  - a departed peer's commands;
  - a stopped peer whose queue stays under 4 KiB while more than fifty states are replaced, and
    whose reliable queue refuses rather than drops;
  - eight command and state breaches, including an inbox flood.

  **Thirteen guards were broken once each and every one failed its own proof.**
- **Found:** admitted commands must be copied, because a departed peer's buffer is reused; the
  per-tick budget needed a limit and batch filling needed an order; "state waits behind the
  baseline" needed a slot outside the queue.
- **The bar is green:** `zig fmt --check`; `zig build test` **82/82 steps, 1,653 of 1,654**
  headless tests (the one skip predates M16) from **1,718 declared**; `check` native, Metal and
  both null cross targets; both samples 30 frames; the optimized Windows checks, Vulkan and null.
  Not run natively on Windows: the change is Zig inside `net`.
- **Not done, deliberately:** no ABI (Step 5); no sample, host modes or presentation maps (Step
  6); no WAN envelope (Step 7).
...
- The package is `demo:warmroom`, an override of the room's `foundry:ui_theme room:ui.theme`.
  ...
  the list controls, a second document, Override Here on the dependency's theme, a refused
  value corrected in place, Undo, Redo, Save All, Validate, Build, Reload and Export. **No
  `.fdt` was written by hand and no source was generated by a script.** The saved files are
  byte-identical between the headless run and a real SDL3/Metal window, and so is the exported
  `.fpk`.
- **Input was replayed, not typed by a hand.** The host reads an action plan from a file it is
  given on its command line (`--plan`); the plan names this package's schemas and fields, and
  the editor still knows none of them. What is not proved is that a person finds the controls.
- The relocated room found `warm-room.fpk` in its ordinary user `mods/` directory and, with it
  enabled, loaded three packages and thirty-three records in the order `requires` demands.
  **Enabling was the host's package list, not a click on Apply**; the mod screen's own path is
  proved by `engine/tests/abi_mod_manager.zig`. The compiled artifact was then granted back as
  a dependency and its `room:ui.theme` copied out through v4: `text_scale 2.5`,
  `line_height 32.0` — the authored values.
- **An external C client does the same job and runs.** `engine/tests/fixtures/author_mod.c`
  creates a document and a `foundry:mod` record, sets the required fields, saves, builds,
  exports and releases; `engine/tests/abi_authoring.zig` is the host that grants it a
  workspace and reads what it wrote, and a second test finds `FOUNDRY_ERR_UNAVAILABLE` from a
  host with no authoring service.
- **Four defects only an outside attempt could find.** `native_loader.zig` still offered
  v1–v3, so a mod declaring `abi { min 4 }` was refused before its image opened; the identity
  read of `mod.fdt` had no `@import` resolver, so a package `fpack` compiles could not be
  opened; choosing a dependency package took effect inside the frame that drew it, listing two
  packages' rows under one set of widget ids; and New Package left the record it had just made
  unselected. All four are fixed, each with a test, and every workflow test now also asserts
  the UI kernel saw no duplicate widget id.
- **Export is new and is not a new call.** `author_build_export` was published in Step 5 and
  unused. The host takes `--export <file.fpk>` and an optional `--export-assets <dir>` — the
  same output set as `fpack --out`/`--assets-out` — and the toolbar has an Export button. The
  client still asks for a destination by number and cannot name a path.
- `docs/modding/editor.md` is written: the grants, the whole workflow from an empty directory
  to an installable `.fpk`, what an override copies, the plan grammar, and the limits.
...
- `author/edit.zig` turns typed, schema-indexed intent into the smallest source splice, parses
  and validates the candidate, and installs it only after all fallible work and history
  reservation succeed. It implements create, duplicate, whole-record dependency override,
  delete, scalar/nested/list set and unset, list insert/remove/move, Undo and Redo. There is no
  file write, package build, ABI publication or Step 4 functionality.
- Every command carries the workspace revision. Stale revisions, dead source positions,
  ill-typed values, duplicate IDs, unspellable hashed IDs and over-budget candidates leave
  source bytes, revision and history unchanged. Successful commands advance the revision;
  dirty state is current bytes versus the independent disk baseline.
- Inspection exposes exact typed values together with authored absence and required, optional
  or default presence. A missing required value is an editable incomplete draft with a
  diagnostic; malformed, unknown-schema and otherwise structurally invalid sources remain
  byte-preserved and read-only.
- Dependency overrides read the `.fpk` field blocks directly rather than using narrowed runtime
  getters. `u64`/`i64` endpoints, `f64` bits, nested/list values and stored absence survive;
  an ID whose source spelling cannot be recovered is refused instead of invented.
- History owns complete before/after fragments and structural selection locators. It is bounded
  by command count and retained bytes, evicts only oldest complete commands with visible
  truncation, refuses a command too large to retain, and clears Redo on a new branch. The
  persistent document budget also counts baselines, drafts, history and the real candidate
  validation peak.
- Parser diagnostics are isolated per parse before being appended to the caller's bounded
  collector. Without that isolation, an earlier missing-dependency diagnostic could make a
  later syntactically valid document look malformed.
- An allocation-failure sweep found and fixed two error cleanups owning the same snapshot array;
  every induced OOM now frees what it acquired and, when it reaches a command, preserves the
  old bytes, revision and history.

The bar passed **1,517 of 1,518** headless tests, with the existing skip, from **1,589
declared**. Six new workspace tests cover every value kind, exact dependency copies, incomplete
drafts, stale/failed atomicity, create/duplicate/delete, history truncation and branching,
one-command history refusal, and allocation failure. Deliberately disabling the revision check
made the stale-command assertion fail; the guard was restored. Resolution: `editor.md`, Step 3.
...
- `ui.Context` accepts an optional borrowed `ui.Skin`. Existing panels, buttons, checks, rows,
  fields and scroll controls draw their fixed nine-slice part when supplied, with the exact flat
  path retained for a missing skin or part.
- `ui/game.zig` adds tabs, selectable rows, opaque images and named icons. The room installs the
  skin resolved from `room:ui.theme` each frame and clears that borrow before releasing it.
- A reorder list overlays caller-drawn rows with grips and an insertion marker, owns no game
  data, and returns the same final-index move for drag or Up/Down/Top/Bottom buttons. Drag
  capture survives leaving the list, and an omitted release cannot leave a phantom gesture.

The bar passed **1,448 of 1,449**, with the existing skip. Deliberately removing downward
destination normalisation made its interaction test return 3 instead of 2; the guard was
restored. Resolution: `mod-management.md`, Step 6.
...
- `ui` gains `image` and `nine_slice` draw commands, naming images by `ui.ImageRef`, an opaque
  `u32`, and `ui.nineSlice`, the cut as pure arithmetic.
- A disabled scope takes no hover, press or focus but still keeps the pointer from the game,
  and fades what it draws by `Style.disabled_alpha`.
- The walker resolves image numbers through `UiDrawOptions.images` and draws nothing for one
  that names nothing.

The bar passed **1,432 of 1,433**, and three mutations were caught and restored. Resolution:
`mod-management.md`, Step 4.
...
Two decisions were proposed, and the owner accepted both the same day:
...
application, per-mod ABI policy, per-save load order and `@patch`. The records the ADRs change
carry dated notes, and `CLAUDE.md` §4.1 indexes both. No code changed.
...
- the bar passed **1,394 of 1,395**;
- headless, 1,382 of 1,383;
- the Metal-selected graph, 1,399 of 1,405, with six expected skips;
- both Vulkan checks and both releases passed.

M13 closes at **1,405 declared / 1,395 headless tests**, and the Vulkan-selected graph declares
1,437, of which 1,427 pass on the Arc. The RHI's rules survived: three were tightened before
Step 4 and none relaxed, and the one extension, out-of-date swapchain rebuilds, is ADR-0037's.
Discrepancies were resolved where they started:
- ADR-0035 and `hardening.md` had handed device recovery to M13; notes now say M13 left it open,
  with loss sticky and `rhi.md` question 6 still open.
- Pacing is recorded as the game's in `app-and-frame-loop.md` §2.
- `jobs-and-threading.md` §9's recording trigger did not fire, at a 0.04 ms median.
- ADR-0008 gains a note, and ADR-0033, ADR-0037 and ADR-0038 say they were implemented.

`CLAUDE.md` §§4 and 9, AGENTS.md, the roadmap, README, the design index, `rhi.md` and
`platform-interface.md` were updated. Tagged `m13`. Resolution: `vulkan.md`, Step 10.
...
an allocator; `jobs.forChunks(len, grain, context, chunkFn)` splits `0..len` into chunks whose
boundaries depend on `len` and `grain` alone and returns when all have run. `serial` runs them in
index order on the caller and is the default; `reversed` runs them backwards, so a call site that
depends on order fails an ordinary test on one thread. A split allocates nothing — its context
lives on the caller's frame, which the join outlives — and its arithmetic cannot overflow at the
top of `u32`. Seven tests, including results folded from slots agreeing under both executors
while a shared running total does not. Breaking the guards failed 5, 2 and 1-then-abort of the
file's 7 tests; nothing else uses it yet. The bar passed. **1,351 declared / 1,341 headless**,
ten Metal-only. Resolution: `jobs-and-threading.md`, Step 1.

**Designed M12 and accepted ADR-0036, 2026-09-13:** the owner accepted the model together with
§8's exit workload and §6.3's sort rule, as proposed.
...
creation — when it is created, at the top of `beginFrame` and after the frame's own clock
reading. Logging therefore reads no clock, and a frame still reads it once. The ring's
`LogRecord` gained `elapsed`; `FoundryLogRecord` is unchanged. The session log is envelope
version 2: its header states the line format, `f<frame> <elapsed> <level>(<scope>): <text>`,
with seconds to the microsecond, or `-` before an engine has observed anything. Opening a
session clears the stamp, and the marker stays version 1. Evidence: 333 frames with both
captures on match 333 with both off in ticks, clock readings and a per-step checksum. Each
sample's 600-frame null run printed identical output with the log on and off, and its log lines
frames up with times from startup to exit. Breaking the guards failed 3, 3 and 3 of 1,333
tests. On Metal with the real clock and API validation, a 300-frame windowed sandbox logged from
`f0 0.000000s` to `f299 2.937981s` with no error. The bar passed. **1,343 declared / 1,333 headless**, ten Metal-only. Resolution:
`hardening.md`, Step 7.

**Implemented in M11 Step 6, 2026-09-13:** the UI's rectangles can share the font's texture.
`foundry:fonts.debug` now keeps solid white in its spare 96th cell, regenerated by
`scripts/gen-debug-font.py` with no other texel changed. `app.UiDrawOptions.solid` draws
rectangles from a region of that texture, which `app.uiSolidRegion` finds from the
`font_solid [ 122 42 4 4 ]` both samples now declare in their `settings` records (schema version
...
through the RHI at once; the device keeps them until the work that used them has finished. The
frame-index queue is gone, and with it the out-of-memory path that leaked a texture;
`Renderer.deinit` no longer idles the device. Uploads stay asynchronous, with no wait added.
Each texture tracks the state its last upload left it in, so adding to an atlas no longer
declares the whole atlas `undefined`, which a backend may discard — the validation backend now
counts such discards for tests. `atlasAdd` claims packer space only after its copy is submitted
(`Packer.fit`, then `commit`), so a failed upload publishes no region and takes no space. **Not
yet:** a recording left open by a failure still holds later retirements until Step 5. Evidence:
through the ordinary registry and texture loader, repeated replacement with frames in flight,
a failed replacement then recovery, and every allocation of a replacement failed in turn;
breaking the three guards failed 13 of 1,306 tests; the Metal-selected tests passed under Metal
API Validation; the null sandbox hot-reloaded its texture 100 times in 3,000 frames with no
violation, and the windowed Metal sandbox 20 times in 600 frames under Metal API Validation
with no error; the bar passed. **1,315 declared / 1,306 headless**, nine Metal-only. Resolution:
`hardening.md`, Step 3.

**Implemented in M11 Step 2, 2026-09-13:** both RHI backends now keep `rhi.md` §3's promise of
deferred destruction. `engine/src/rhi/lifetime.zig` is the one completion model they share:
recordings numbered as they begin, submissions as they reach the queue, and a destroyed
resource's backing retained until every recording that could use it has finished. A slot's
wait covers everything submitted before that slot's frame ended, uploads outside a frame
included; `waitIdle` covers the newest submission; ending a frame covers nothing. Destroy never
allocates, because creation reserves the retirement first. Rule 9 now rejects recording through
a dead handle — directly, or through a live bind group that names something dead — instead of
rejecting the destroy, and pipelines copy what they need from their layouts. Metal holds
submitted command buffers until a wait covers them rather than relying on Metal's own retention,
and two double-release paths in its `Device.init` are gone. **Not yet:** the renderer still
keeps its own frame-index retirement layer (Step 3), and until Step 5 a recording left open by
an error path holds later retirements until teardown — memory, not safety. Evidence: an
allocation-failure sweep over every resource kind; breaking retirement and the dead-handle
check failed 10 of 1,299 tests; the Metal-selected tests passed under Metal API Validation;
the bar passed. **1,308 declared / 1,299 headless**, nine Metal-only. Resolution:
`hardening.md`, Step 2.
...
potentially raising Lua operation behind `lua_pcall`; the Zig side supplies a stable,
caller-owned allocator context, while the C allocator enforces the initial 8 MiB VM quota and
the instruction hook enforces the 100,000-instruction default. The deliberately small fixture
environment is constructed by allowlist and exposes no OS, package, debug, coroutine, dynamic
load or protected-call escape. The fixture executes text and returns one integer solely to
prove the boundary; it is not the M8 author contract.

**Runtime evidence:** 14 focused tests execute text; reject forbidden globals and binary/NUL
...
- the qualified Windows target and pinned tools;
- native window payloads and the system-library open;
- a validated device with resources, checked SPIR-V, persistent bindings and pipelines;
- offscreen drawing, and presentation with resize and failed-frame closure;
- both samples, with their icons;
- Windows' proof: a RenderDoc capture, pacing and real input;
- the closing gate.

The RHI's rules survived with none relaxed. Linux left the milestone for M18 (ADR-0039). M13
closed at **1,405 declared / 1,395 headless tests**, ten Metal-only, with 1,437 in the
Vulkan-selected graph, and is tagged `m13`.
...
**`core` (L0), `platform`, `data`, `physics2d` and `ui` (L1), `rhi` (L2) with three backends —
validation, Metal and Vulkan —
...
**At M13's close, 2026-09-19:** `zig build test` passes 1,394 of 1,395 tests on the Mac, 1,405
are declared under `-Drhi=metal`, and on Windows the Vulkan-selected graph passes 1,427 of 1,437.
Both samples run on macOS through Metal and on Windows x64 through Vulkan, from a relocated
install. The per-module figures below are older and kept as history.
...
explicit discard/refresh, stable snapshots and retained private build candidates. Step 5
published all of it as the additive `FoundryApi_v4` and moved `fpack` onto that table. Step 6
...
- Native builds there use at most two jobs at below-normal priority, because it is the owner's
  gaming PC.
...
* **M13 — Portable.** **Complete, 2026-09-19, tagged `m13`**; Windows x64 only.
...
* **Vulkan is proven on Windows x64 alone, and on one machine** (M13, 2026-09-19). The claim
  ...
  - Pacing a skipped frame is the game's (`app-and-frame-loop.md` §2). A minimised sample still
    spends about a quarter of a core on its own frames' work, and a minimised Metal window's
...
5. ~~**When backend #2 is triggered.**~~ **Settled.** The owner triggered it on 2026-09-14 to
   validate the RHI (ADR-0033), and M13 completed it on 2026-09-19. Vulkan runs on Windows x64,
   and the RHI's rules survived with none relaxed.
