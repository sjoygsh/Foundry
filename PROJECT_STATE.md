...
**Updated by:** **M2: designed, and `asset` built.** The design doc, the two decisions M2
forced, the clip-space contract M1 left undocumented, and Foundry's own PNG decoder
...
**M2 is under way.** Two of the six steps below are done: the clip-space contract is
written down, and `asset` exists with the PNG decoder ADR-0018 called for. 245 tests pass
under `-Drhi=null` and 253 under `-Drhi=metal`; all eight target/backend combinations
compile. Nothing draws a sprite yet — `render2d` has not been started.
