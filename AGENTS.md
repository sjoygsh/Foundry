...
Windows zip is run on a machine with no Foundry, Zig or Vulkan SDK present, and the macOS zip in
a fresh macOS VM (UTM, from Apple's restore image); the owner's PC was
