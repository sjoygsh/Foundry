...
Accepted [ADR-0037](adr/0037-vulkan-execution-and-presentation.md) covers Vulkan 1.3,
...
profiles on disk, one file each, kept in the player's order, with consent per version.
...
walker's table, with the room's card drawn from `room:ui.theme`. **Step 6:** the existing
widgets draw from that skin, and `tabs`, `selectable`, reorder, `icon` and `image` complete the
...
### M15 — Editor: "content is authored in Foundry" — **in progress: Steps 1–4 of 9 done**
...
author/consumer proof; closure. Steps 1–4 are done (2026-09-20): the parser's opt-in source spans
...
build candidates byte-identical to `fpack`. The first editor authors records using existing
schemas, preserves source text, and proves its output through the sample's normal mod path. A
scene editor, schema designer and executing game/plugin host are outside this scope.
