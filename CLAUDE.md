...
| Platforms | macOS/Apple Silicon primary; Windows x64 runtime-proven on Vulkan in M13; Linux x64 headless/server runtime-proven in M16.5, desktop build-checked until M18 | [0008](docs/adr/0008-target-platforms.md), with [0033](docs/adr/0033-vulkan-second-backend.md), [0039](docs/adr/0039-linux-after-the-first-game.md) and [0046](docs/adr/0046-linux-headless-servers-before-release.md) |
...
| Distribution | Relocatable applications around ordinary packages; explicit staging for macOS and Windows; unsigned GitHub pre-releases until certification, which waits for a fully playable 3D game | [0030](docs/adr/0030-distribution-artifacts.md), with [0032](docs/adr/0032-defer-macos-release-certification.md) and [0047](docs/adr/0047-unsigned-github-preview-release.md) |
...
L2  net         -> core, platform.  M16's runtime channels, FNET wire codec and
                                sessions — grants, admission, negotiation, allowlist,
                                limits, deadlines, baselines, tick-admitted command
                                batches, replaceable state — over `platform`'s
...
**M16 (complete 2026-09-23, tag `m16`):**
...
an optional `net` module and eventually an additive public ABI. **The owner requires
...
deterministic memory carrier for `net`'s tests. Step 3 added `net.Service`: sessions only by
host grant, an allowlist mapping keys to principals, compatibility refused by category and first
difference, bounded pre-authentication work, deadlines, and events reserved so none is dropped.
...
Step 7 proved that consumer against forged commands, a lying server, revoked, rotated and
...
p95 command acknowledgement near 230 ms against a 500 ms budget. Step 8 played it over the
public internet, macOS and Windows clients on separate access networks against a cloud-hosted
authority (p95 68–118 ms), and wrote `docs/modding/networking.md` from an external C99
consumer. Step 9 closed it. **The durable result is that Foundry plays over the public
internet through its one public table**, with every peer authenticated and the server the
only authority.
...
Recorded so they are not made accidentally. Each notes when it comes due. **A postponed
decision becomes its own milestone rather than being squeezed into a current one**; the
milestone named below is where `docs/ROADMAP.md` now places it.
...
| Linux runtime support | **M18**, trigger-started — after the first game is complete, before any 3D | **Removed from M13 by ADR-0039, 2026-09-18.** The first game targets macOS and Windows. The Linux Vulkan paths are written and build-checked; no X11, Wayland or Linux driver has run them. **Linux headless use (the first game's servers) was pulled forward by ADR-0046 and proven in M16.5 (2026-09-23)**; the desktop stays here. |
