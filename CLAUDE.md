...
**M16 planning (proposed and revised 2026-09-21):**
...
an optional `net` module and an additive public ABI. **The owner requires public-internet
multiplayer**; the LAN-only proposal is withdrawn. TLS 1.3 mutual authentication, operator-hosted
sessions and bounded credential/admission handling are the revised proposal.
They are **not accepted decisions** and do not change §4.3's implemented graph. The owner must
accept the remaining model, topology, admission UX and performance choices before Step 1.
Internet security and a real WAN proof are M16 requirements, not deferred release polish.
...
| Networking | **M16**, public-internet trigger confirmed; design proposed | Owner requires internet multiplayer. [networking.md](docs/design/networking.md) and proposed ADR-0044/0045 await remaining architectural acceptance; no implementation has begun. |
