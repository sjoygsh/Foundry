...
test "get_api hands out v1 to v5 side by side and refuses unknown versions" {
    ...
    const v4 = Table.getApi(4) orelse return error.TestUnexpectedResult;
    const typed_v4: *const Api_v4 = @ptrCast(@alignCast(v4));
    ...
    inline for (@typeInfo(Api_v3).@"struct".fields) |field| {
        if (comptime isCall(field.type)) {
            try testing.expect(@field(table_v3, field.name) == @field(table_v4, field.name));
